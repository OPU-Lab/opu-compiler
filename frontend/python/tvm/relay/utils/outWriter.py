    ...
    def get_mac_number(self, ir): # fused weights
        ...
        for i in range(ir['num'][0]):
            ...
            if type == 1: #conv2d
                ...
                print('\ninput:',input_size,'out_nopool:',[Do, Wo, Ho ,Cout],'pad:',padding_size,'stride:',ker_stride,'kz:',ker_size,'group:',group)
                if group == 1:
    ...
    def write2file_opu_ir(self):
        ...
        for i in range(len(self.layer_infos)):
            ...
            if len(input)>1:
                ...
                extra_concat_input_ids = [x+1 for x in layer['extra_concat_inputs']]
                residual_inputs = []
                if layer['residual_input_layer_id'] is not None:
                    residual_inputs.append(layer['residual_input_layer_id']+1)
            ...
            # accurate padding
            #if len(ir['input_size'][i])==4:#2d
            #    p = layer['padding_size']
            #    ir['pre_padding_size'][i] = [p[1][0],p[1][1],p[2][0],p[2][1],0] #udlr                 
            # fix_point
            # TODO
            # upsampling_scale
            ir['upsampling_scale'].append(int(layer['upsampling_scale']))
        ...
        # old version
        ir['input_size_sw'] = [x[1:] for x in ir['input_size']]
        ir['sw_input_size_after_padding'] = [x[1:] for x in ir['input_size']]
        ir['output_size_sw'] = [x[1:] for x in ir['output_size']]
        ir['post_padding'] = [0 for i in range(len(self.layer_infos))]
        ir['post_padding_size'] = [[0,0,0,0] for i in range(len(self.layer_infos))]  
        ir['sw_pre_padding_size'] = [[0,0,0,0] for i in range(len(self.layer_infos))]  
        for ii in range(ir['num'][0]):
            #print('='*10,ii)
            #print(ir['input_size'][ii])
            #print(ir['ker_size'][ii])
            #print(ir['ker_stride'][ii])
            #print(ir['pre_padding_size'][ii])
            N,H,W,C = ir['input_size'][ii]
            _,Kh,Kw = ir['ker_size'][ii]
            _,Sh,Sw = ir['ker_stride'][ii]
            ...
            Pw_r = int(Pw//2)
            Pw_l = int(Pw-Pw_r)
            ir['sw_pre_padding_size'][ii] = [Ph_l,Ph_r,Pw_l,Pw_r]
            ir['sw_input_size_after_padding'][ii] = [N, H+Ph_l+Ph_r, W+Pw_l+Pw_r, C]
            Ph_r = Ph_r = max(Ph_r, Ph_l)
            Pw_r = Pw_r = max(Pw_r, Pw_l)
            #print([Ph_l,Ph_r,Pw_l,Pw_r])
            ir['pre_padding_size'][ii] = [Ph_l,Ph_r,Pw_l,Pw_r]
            ir['input_size'][ii] = [N, H+Ph_l+Ph_r, W+Pw_l+Pw_r, C]
            #print(ir['input_size'][ii])
        ir['pre_remove_padding'] = [[0,0,0,0] for ii in range(ir['num'][0])]
        for ii in range(ir['num'][0]):
            #print('='*10,ii)
            #print('output:',ir['output_size'][ii],[ir['input_size'][j-1] for j in ir['output_ddr_addr'][ii]])
            inz = [ir['input_size'][j-1][1:3] for j in ir['output_ddr_addr'][ii]]
            idx = int(np.argmax(np.sum(np.array(inz),axis=1)))
            #print(idx)
            if -1 not in ir['output_ddr_addr'][ii]:
                ir['output_size'][ii][1:3] = inz[idx]
                idx = ir['output_ddr_addr'][ii][idx]-1 
                ir['post_padding_size'][ii] = ir['pre_padding_size'][idx]
                ir['post_padding'][ii] = ir['pre_padding'][idx]
            for j in ir['output_ddr_addr'][ii]:
                ir['pre_remove_padding'][j-1] = [ir['post_padding_size'][ii][p] - \
                    ir['sw_pre_padding_size'][j-1][p] for p in range(4)]
                if j==-1:
                    continue
                ir['input_size'][j-1][1:3] = ir['output_size'][ii][1:3]
        '''for ii in range(ir['num'][0]):
            print('='*10,ii)
            for j in ir['output_ddr_addr'][ii]:
                print(ir['output_size'][ii],'->',ir['input_size'][j-1],ir['pre_remove_padding'][j-1])'''
        ...
        for ii in range(ir['num'][0]):
            ...
            if ir['activation_type'][ii]==2: 
                ir['activation_type'][ii]=1
        # remove shortcut source from input_ddr_addr, which is only for concat
        for ii in range(ir['num'][0]):       
            for item in ir['shortcut_source'][ii]:
                ir['output_ddr_addr'][item-1].remove(ii+1)
        #ir.pop('upsampling_scale')   
        ir.pop('pre_padding')   
        ir.pop('pre_padding_size')   
        ir.pop('sw_pre_padding_size')   
        ir.pop('input_size_sw')   
        ir.pop('output_size_sw')   
        ir.pop('group')   
        ir.pop('channel_shuffle')   
        ir.pop('sw_input_size_after_padding')   
            
        with open(os.path.join(self.out_dir,'IR.json'),'w') as fp:
            json.dump(ir, fp)        
