...
def _conv(opname):
    def _impl(inputs, attr, params):
        ...
        if attr['padding'] == 'VALID':
            ...
            dilation_h = attr['dilations'][0] #TOTO
            dilation_w = attr['dilations'][1] #TOTO
...
def _bias_add():
    def _impl(inputs, attr, params):
        ...
        if attr['data_format'].decode("utf-8") == 'NCHW': #TOTO
...
def from_tensorflow(graph, layout="NHWC", shape=None, outputs=None):
    ...
    '''import ipdb
    #ipdb.set_trace()
    inp_node = [x for x in graph.node if x.op=='Placeholder'][0]
    inp_node.name = 'input'
    inp_node.attr['shape'].shape.dim[1].size=368
    inp_node.attr['shape'].shape.dim[2].size=368
    inp_node.attr['shape'].shape.dim[3].size=3
    conv = [x for x in graph.node if 'image' in x.input][0]
    weight = conv.input[1]
    conv.ClearField('input')
    conv.input.append('input')
    conv.input.append(weight)'''
    g = GraphProto()
