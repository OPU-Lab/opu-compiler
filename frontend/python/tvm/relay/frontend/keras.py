...
def _convert_convolution1d(inexpr, keras_layer, etab):
    ...
    for i in range(len(weightList)):
        weightList[i] = np.expand_dims(weightList[i], 0)
    ...
    if isinstance(keras_layer.dilation_rate, (list, tuple)):
        dilation = [keras_layer.dilation_rate[0], keras_layer.dilation_rate[0]]
    else:
    ...
    stride_h, stride_w = keras_layer.strides[0], keras_layer.strides[0]
    ...
    if keras_layer.use_bias:
        bias = etab.new_const(weightList[1].squeeze())
        out = _op.nn.bias_add(out, bias)
    # defuse activation
    if sys.version_info.major < 3:
        act_type = keras_layer.activation.func_name
...
def _convert_pooling1d(inexpr, keras_layer, etab):
    ...
    pool_h, pool_w = 1, keras_layer.pool_size[0]
    stride_h, stride_w = 1, keras_layer.strides[0]
...
def _convert_padding1d(inexpr, keras_layer, _):
    _check_data_format(keras_layer)
    padding_type = type(keras_layer).__name__
    padding = keras_layer.padding
    top = left = bottom = right = 0
    if padding_type == 'ZeroPadding1D':
        if isinstance(padding, int):
            top = left = bottom = right = padding
        elif isinstance(padding, tuple):
            if isinstance(padding[0], int):
        ...
        raise tvm.error.OpNotImplemented(msg.format(padding_type))
    return _op.nn.pad(data=inexpr,
                      pad_width=((0, 0), (0, 0), (0, 0), (left, right)))
...
_convert_map = {
    ...
    'ZeroPadding1D'          : _convert_padding1d,
...
def from_keras(model, shape=None):
    ...
    #Sequential model
    import keras
    tmp = [(lambda x:1 if isinstance(x, keras.engine.sequential.Sequential) else 0)(x) for x in model.layers]
    is_sequential_model = not sum(tmp)==0
    if is_sequential_model:
        #update input for 1st layer in Sequential() # assume 1 input and 1 Sequential
        inp = [x for x in model.layers if isinstance(x, keras.engine.input_layer.InputLayer)][0]
        first_layer_in_sequential = [x for x in model.layers if isinstance(x, keras.engine.sequential.Sequential)][0].layers[0]
        first_layer_in_sequential._inbound_nodes[0].inbound_layers.clear()
        first_layer_in_sequential._inbound_nodes[0].inbound_layers.append(inp)
        #flatten Sequential Node
        tmp = []
        for x in model.layers:
            if isinstance(x, keras.engine.sequential.Sequential):
                tmp+=x.layers
            else:
                tmp.append(x)
        model.layers.clear()
        for x in tmp:
            model.layers.append(x)
        #update model._network_nodes i.e. 'sequential_1_ib-1', 'input_1_ib-0'
        model._network_nodes.clear()
        for i,layer in enumerate(model.layers):
            name = layer.name+'_ib-0'
            model._network_nodes.add(name)
        #update model._output_coordinates
        tmp = []
        for x in model._output_coordinates:
            if isinstance(x[0], keras.engine.sequential.Sequential):
                tmp.append((x[0].layers[-1],0,x[2]))
            else:
                tmp.append(x)
        model._output_coordinates.clear()
        for x in tmp:
            model._output_coordinates.append(x)
