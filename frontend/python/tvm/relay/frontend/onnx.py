...
def dimension_picker(prefix, surfix=''):
    ...
    def _impl(attr):
        ...
        msg = 'Only 1D and 2D kernels but {}D are supported for operator {}.'
        op_name = prefix + '1d/2d'
        raise tvm.error.OpAttributeInvalid(msg.format(len(kernel), op_name))
...
class Upsample(OnnxOpConverter):
    ...
    def _impl_v9(cls, inputs, attr, params):
        ...
        if not scales:
            ...
            scales = [1,1,2,2]#params[inputs[1].name_hint].asnumpy() # for yolov3
            inputs = inputs[:1]
