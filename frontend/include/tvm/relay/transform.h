...
TVM_DLL Pass CanonicalizeConcatPos();
TVM_DLL Pass FuseOpHW();
TVM_DLL Pass FuseOpHWCustom();
TVM_DLL Pass ExplicitPad();
TVM_DLL Pass GenIR(bool quantize = false, bool use_post_padding = false);
TVM_DLL Pass ToNCHWLayout();
TVM_DLL Pass Peephole();
TVM_DLL Pass ToJson(bool reuse_input = false);
}  // namespace transform
